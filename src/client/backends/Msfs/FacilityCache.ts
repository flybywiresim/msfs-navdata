...
export class FacilityCache {
    ...
    public async searchByIdent<T extends IcaoSearchFilter>(ident: string, type: T, maxItems: number): Promise<SearchedFacilityType<T>> {
        ...
        if (type === IcaoSearchFilter.Intersections) {
            return [...(await this.getFacilities(icaos, LoadType.Intersection)).values()] as SearchedFacilityType<T>;
        }

        if (type === IcaoSearchFilter.Ndbs) {
            return [...(await this.getFacilities(icaos, LoadType.Ndb)).values()] as SearchedFacilityType<T>;
        }

        if (type === IcaoSearchFilter.Vors) {
            return [...(await this.getFacilities(icaos, LoadType.Vor)).values()] as SearchedFacilityType<T>;
        }
